...
public class Assert_Exercise extends Init{
	@Test
	public void checkTruFail() throws InterruptedException {
		...
		WebElement pagesubmityourlistElement = driver.findElement(By.xpath("//h2[contains(text(),'Submit Your Listing')]"));
//		Kiểm tra đúng với thư viện Assert
		Assert.assertEquals(pagesubmityourlistElement.getText(), "Submit Your Listing");
//		Kiểm tra chứa với thư viện Assert
		Assert.assertTrue(pagesubmityourlistElement.getText().contains("Submit Your"));
...
//		Hỗ trợ bấm Enter
		...
		Assert.assertEquals(regionElement.getText(), "Boston");
...
//		1. Price Range
		...
		Assert.assertEquals(getnamepriceyElement.getText(), "$$ Moderate");
...
//			4.1 time openn Monday 
		...
		Assert.assertEquals(openMondayElementtt.getText(), "07:30 am");
...
//		4.2 time close Monday 
	...
	Assert.assertEquals(closeMondayElementtt.getText(), "09:45 pm");
